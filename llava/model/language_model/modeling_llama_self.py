...
class VLMPruner(LlamaModel):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if VLMPruner_config is not None:
                    ...
                    if decoder_layer.self_attn.layer_idx == K and seq_length > 1:
                        ...
                        # print(last_layer_state.shape) # torch.Size([1, 638, 4096])
                        # print(any_states.shape) # torch.Size([1, 32, 638, 128])
                        ...
                        # keep index
                        retained_image_tokens_index, hidden_states = self.get_retained_image_token(self.config, last_layer_state, any_states)
    ...
    # 需要添加的辅助函数
    def get_spatial_pos(self, indices):
        width = 24  
        y = indices // width
        x = indices % width
    ...
    def get_retained_image_token(self, config, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
        self.norm_factor = 1.0 / (24*math.sqrt(2))
        VLMPruner_config = config.VLMPruner_config
        image_token_start_index = VLMPruner_config['image_token_start_index']
        image_token_length = VLMPruner_config['image_token_length']
        pivot_image_token = VLMPruner_config['pivot_image_token']
        retain_token_num = round(image_token_length * (1 - VLMPruner_config['reduction_ratio']))
        ...
        spatial_all = torch.cdist(self.get_spatial_pos(torch.arange(image_token_length, device=device)),
                                    self.get_spatial_pos(torch.arange(image_token_length, device=device)))
        # Step 2
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # global prefill
        # # decoder outputs consists of (dec_features, layer_state, dec_hidden, dec_attn)
        # b = time.time() - self.model.config.start
        # if self.model.config.zero == 0:
        #     prefill = prefill + b
        # print(prefill, self.model.config.count, prefill / self.model.config.count)
        # self.model.config.zero = 1
