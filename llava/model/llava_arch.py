...
def unpad_image(tensor, original_size):
    ...
    # 检查像素数量限制
    # output_height, output_width = unpadded_tensor.shape[1:]
    # current_pixels = output_height * output_width
    # max_pixels = 576 * 3
    ...
    #     # 计算新的尺寸
    #     new_height = max(1, int(output_height * scale_ratio))
    #     new_width = max(1, int(output_width * scale_ratio))

    #     # 使用插值进行缩放
    #     unpadded_tensor = torch.nn.functional.interpolate(
    #         unpadded_tensor.unsqueeze(0),  # 添加batch维度
...
class LlavaMetaForCausalLM(ABC):
    ...
    def DivPrune(self, visual_feature_vectors, image_feature_length, cosine_matrix=None):     
        threshold_terms = int((1-float(os.environ['RATIO'])) * self.config.VLMPruner_config["image_token_length"]) # int(round(threshold_ratio*image_feature_length))
    ...
    def prepare_inputs_labels_for_multimodal(
                ...
                for image_idx, image_feature in enumerate(image_features):
                    if image_feature.shape[0] > 1:
                        ...
                        if 'unpad' in mm_patch_merge_type:
                            ...
                            self.config.VLMPruner_config["width"] = image_feature.shape[1]
                            self.config.VLMPruner_config["height"] = image_feature.shape[2]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    self.config.VLMPruner_config["image_token_length"] = cur_image_features.shape[0]
        ...
        if 'LAYER_INDEX' in os.environ and os.environ['LAYER_INDEX']=='0':
            ...
            selected_visual_tokens, cosine_matrix = self.DivPrune(visual_tokens, img_feature_len,cosine_matrix)
