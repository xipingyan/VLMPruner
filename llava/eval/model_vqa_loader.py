...
def configure_VLMPruner(model, args):
    ...
    if args.sparse:
        ...
        model.config.VLMPruner_config = VLMPruner_config

    else:
        model.config.VLMPruner_config = None
    model.config.count = 0
...
def eval_model(args):
    ...
    configure_VLMPruner(model, args) # HACK: configure DART
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        model.config.start = time.time()
        model.config.count = model.config.count + 1
        model.config.zero = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument('--token_batch', type=int, default=16, help='token_batch')
